...
from sqlalchemy import ForeignKey, Result, select, and_
...
class TaskLinked(Base):
	...
	def to_json(self) -> TaskLinkedModel:
		return TaskLinkedModel(id=self.id, task_id=self.task_id, linked_id=self.linked_id)
...
class TaskLinkedAdapter(DBAdapter):
	...
	async def find_link(self, payload: TaskLinkedAddModel) -> TaskLinkedModel | None:
		stmt = select(self.model).where(
			and_(self.model.task_id == payload.task_id, self.model.linked_id == payload.linked_id)
		)
	...
	async def get_linked_tasks_ids(self, task_id: int) -> list[int]:
		...
		res: Result = await self.session.execute(stmt)
		tasks_linked_ids = [task_linked_row.to_json().linked_id for task_linked_row in res.scalars().all()]
